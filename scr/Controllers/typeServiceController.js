const typeServiceServices = require('../Services/typeServiceServices');
...
const registerController = async (req, res, next) => {
 ...
 const _id = req.headers['x-access-token'];
  ...
  try {
    const response = await typeServiceServices.registerService(data, _id);
...
const editController = async (req, res, next) => {
  ...
  try {
    ...
    const response = await typeServiceServices.editSevice(token, data, idTypeService);
...
const deleteUserController = async (req, res, next) => {
  ...
  try {
    ...
    const response = await typeServiceServices.deleteUserService(_id, token);
    if (response.code > 0 && response.title === 'Error') {
      const exception = new Error(response.message);
      exception.code = response.code;
      throw exception;
...
const listTypeServiceController = async (req, res, next) => {
  const token = req.headers['x-access-token'];

  try {
    const response = await typeServiceServices.listTypeServiceService(token);
    if (response.code > 0 && response.title === 'Error') {
      const exception = new Error(response.message);
      exception.code = response.code;
      throw exception;
...
const getTypeServiceController = async (req, res, next) => {
  const token = req.headers['x-access-token'];
  const idTypeService = req.params.id;
  try {

    const response = await typeServiceServices.getTypeServiceService(idTypeService, token);
    if (response.code > 0) {
      const exception = new Error(response.message);
      exception.code = response.code;
      throw exception;
...
module.exports = {
  ...
  listTypeServiceController,
  getTypeServiceController,
};